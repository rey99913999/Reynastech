    ...
    override suspend fun resolve(target: String): DeviceTargetResolution? {
        ...
        val candidates = nodes.mapNotNull { node ->
            ...
            DeviceTarget(
                ...
                confidence = if (
                    normalizeDeviceText(node.text?.toString().orEmpty()) == needle
                ) {
                    1f
                } else {
                    0.94f
            ...
            it.boundsLeft.toString() + ":" +
                it.boundsTop + ":" + it.boundsRight + ":" + it.boundsBottom
        }
...
class DeviceTargetResolver(
    ...
    suspend fun resolve(
        ...
        return accessibilityAmbiguous
            ?: DeviceTargetResolution.NotFound(target, "target_not_found")
    }

    private fun parseCoordinates(value: String): DeviceTarget? {
        val match = Regex("""^\(?\s*(\d+(?:\.\d+)?)\s*[, ]\s*(\d+(?:\.\d+)?)\s*\)?$""")
            .matchEntire(normalizeDeviceText(value))
            ?: return null
